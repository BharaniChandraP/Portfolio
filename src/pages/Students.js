...
// Helper components defined at the top to avoid initialization errors
const CommitteeCard = ({ univ, scholar }) => (
  <Box p={6} bg="white" border="1px solid" borderColor="gray.100" borderRadius="xl" boxShadow="md">
    <Text fontSize={{ base: "xs", lg: "sm" }} color="blue.600" fontWeight="800" mb={1}>{univ}</Text>
    <Text fontWeight="bold" fontSize={{ base: "sm", lg: "lg" }} color="gray.800">{scholar}</Text>
  </Box>
);

const StatBox = ({ num, label, sub }) => (
  <Box p={10} bg="blue.50" borderRadius="3xl" textAlign="center" border="1px solid" borderColor="blue.100">
    <Heading size={{ base: "xl", lg: "2xl" }} color="blue.700">{num}</Heading>
    <Text fontSize={{ base: "sm", lg: "md" }} fontWeight="800" mt={2} textTransform="uppercase">{label}</Text>
    <Text fontSize={{ base: "10px", lg: "xs" }} color="gray.500" mt={1}>{sub}</Text>
  </Box>
);
...
const Students = () => {
  return (
    <Box>
      <Heading size={{ base: "xl", lg: "2xl" }} mb={6} color="#0a192f">Mentorship & Supervision</Heading>
      ...
      <Box mb={16}>
        <Flex align="center" mb={8}>
          <Box color="blue.600" mr={4} fontSize="24px"><FaUniversity /></Box>
          <Heading size={{ base: "md", lg: "lg" }} color="blue.800" textTransform="uppercase" letterSpacing="widest">
      ...
      {/* Thesis Reviews & Examinations */}
      <Box mb={16} p={10} bg="blue.900" color="white" borderRadius="3xl" boxShadow="2xl">
        <Flex align="center" mb={6}>
          <Box color="blue.200" mr={4} fontSize="24px"><FaMicroscope /></Box>
          <Heading size={{ base: "sm", lg: "md" }}>Thesis Examination & Review</Heading>
        </Flex>
        <VStack align="start" spacing={5}>
          <Text fontSize={{ base: "sm", lg: "lg" }} lineHeight="tall">
            • Reviewed MS Thesis: "Automated recognition of precast structural elements from point cloud data for progress monitoring using BIM and Lidar" (IIT Palakkad).
          </Text>
          <Text fontSize={{ base: "sm", lg: "lg" }} lineHeight="tall">
            • External Examiner for Master's degree Thesis at IIT Madras and IIT Palakkad.
          </Text>
          <Text fontSize={{ base: "sm", lg: "lg" }} lineHeight="tall">
            • Invited peer-reviewer for Emerald's Engineering, Construction and Architectural Management journal.
          </Text>
        </VStack>
      </Box>
      ...
      {/* Mentorship Stats */}
      <Box>
        <Heading size={{ base: "md", lg: "lg" }} mb={8} color="blue.800" textTransform="uppercase" letterSpacing="widest">
          Student Guidance Overview
        </Heading>
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={8}>
          <StatBox num="04" label="M.Tech Guided" sub="Completed (2024-25)" />
          <StatBox num="08" label="B.Tech Guided" sub="Completed (2024-25)" />
          <StatBox num="10" label="Ongoing Scholars" sub="PhD, M.Tech & B.Tech" />
        </SimpleGrid>
      </Box>
    </Box>
