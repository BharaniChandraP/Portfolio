  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={12} wrap="wrap" gap={6}>
        <Box>
          <Heading size={{ base: "xl", lg: "2xl" }} color="#0a192f">Research & Publications</Heading>
          <Text fontSize={{ base: "md", lg: "xl" }} color="gray.600" mt={2}>
            Complete bibliography including Journals, Books, and Industry Reports
          </Text>
        </Box>
        <Link href="https://scholar.google.com/citations?user=MSQauoMAAAAJ&hl=en" target="_blank" style={{ textDecoration: 'none' }}>
          <Button colorScheme="blue" leftIcon={<SiGooglescholar />} size={{ base: "md", lg: "lg" }} px={8}>
            Google Scholar
          </Button>
        </Link>
      </Flex>
      ...
      <VStack align="stretch" spacing={16}>
        ...
        <Box>
          <SectionHeader title="Awarded Conference Papers" icon={FaAward} />
          <VStack align="stretch" spacing={6}>
            <Box p={8} bg="yellow.50" borderRadius="2xl" border="1px solid" borderColor="yellow.200" boxShadow="sm">
               <Badge colorScheme="yellow" mb={3} fontSize="xs" px={3} py={1} borderRadius="full">Best Jury Paper Award</Badge>
               <Text fontWeight="800" fontSize={{ base: "md", lg: "xl" }} color="gray.800">Labor sub-contracting issues in the Indian construction industry</Text>
               <Text fontSize={{ base: "sm", lg: "md" }} color="gray.600" mt={2}>PMI Research and Academic Conference, IIT Delhi (2017)</Text>
            </Box>
            <PubItem 
              title="Digitalising lean construction with Construction 4.0" 
              sub="Indian Lean Construction Conference (2021)"
...
const SectionHeader = ({ title, icon }) => (
  <Flex align="center" mb={10} borderBottom="2px solid" borderColor="blue.50" pb={4}>
    <Box color="blue.600" mr={4} fontSize="24px"><Icon as={icon} /></Box>
    <Heading size={{ base: "md", lg: "lg" }} color="blue.900" textTransform="uppercase" letterSpacing="widest">{title}</Heading>
  </Flex>
);

const PubItem = ({ title, sub, link }) => (
  <Box p={6} borderRadius="2xl" _hover={{ bg: "blue.50" }} transition="0.2s" border="1px solid" borderColor="transparent" hoverBorderColor="blue.100">
    <Text fontWeight="bold" fontSize={{ base: "md", lg: "xl" }} color="gray.800" lineHeight="1.4">{title}</Text>
    <Text fontSize={{ base: "sm", lg: "lg" }} color="blue.600" mt={1} mb={3}>{sub}</Text>
    {link && (
      <Link href={link} isExternal fontSize={{ base: "xs", lg: "sm" }} color="gray.500" fontWeight="bold">
        VIEW PUBLICATION <FaExternalLinkAlt style={{display: 'inline', marginLeft: '6px'}} />
      </Link>
    )}
  </Box>
);
...
const ReportCard = ({ title, org, link }) => (
  <Box p={8} bg="gray.50" borderRadius="2xl" border="1px solid" borderColor="gray.100" boxShadow="sm" _hover={{ boxShadow: "md" }} transition="0.2s">
    <Text fontWeight="800" fontSize={{ base: "md", lg: "lg" }} color="gray.700" mb={2}>{title}</Text>
    <Text fontSize={{ base: "sm", lg: "md" }} color="gray.500" mb={4}>{org}</Text>
    <Link href={link} isExternal color="blue.600" fontSize={{ base: "xs", lg: "sm" }} fontWeight="bold">
      ACCESS REPORT <FaExternalLinkAlt style={{display: 'inline', marginLeft: '6px'}} />
    </Link>
  </Box>
);
